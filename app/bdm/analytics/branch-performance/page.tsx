...
export default function BranchPerformancePage() {
      ...
      case 'custom':
        return {
          ...
          label: customStart && customEnd
            ? `${format(new Date(customStart), 'MMM dd')} - ${format(new Date(customEnd), 'MMM dd, yyyy')}`
            : 'Select dates'
        }
    }
  }
    ...
    try {
      ...
      const [stockReports, revenueReports, occupancyReports, stockItems] = await Promise.all([
        supabase.from('stock_inventory_reports').select('*')
          .gte('report_date', start).lte('report_date', end).eq('status', 'approved'),
      ...
      const branchGroups: Record<string, Manager[]> = {}
      
      typedManagers.forEach(manager => {
      ...
      for (const [branchName, branchManagers] of Object.entries(branchGroups)) {
        ...
        const branchStockReports = (stockReports.data || []).filter((r: Record<string, unknown>) =>
          managerIds.includes(r.manager_id as string)
        )
        ...
        const branchRevenueReports = (revenueReports.data || []).filter((r: Record<string, unknown>) =>
          managerIds.includes(r.manager_id as string)
        )
        const roomRevenue = branchRevenueReports.reduce((sum: number, r: Record<string, unknown>) => sum + ((r.room_revenue as number) || 0), 0)
        const laundryRevenue = branchRevenueReports.reduce((sum: number, r: Record<string, unknown>) => sum + ((r.laundry_revenue as number) || 0), 0)
        const otherRevenue = branchRevenueReports.reduce((sum: number, r: Record<string, unknown>) => sum + ((r.other_services_revenue as number) || 0), 0)

        const totalRevenue = salesRevenue + roomRevenue + laundryRevenue + otherRevenue

        const branchOccupancy = (occupancyReports.data || []).filter((r: Record<string, unknown>) =>
          managerIds.includes(r.manager_id as string)
        )
        const avgOccupancy = branchOccupancy.length > 0
          ? branchOccupancy.reduce((sum: number, r: Record<string, unknown>) => sum + ((r.occupancy_percentage as number) || 0), 0) / branchOccupancy.length
          : 0

        const branchItems = (stockItems.data || []).filter((item: Record<string, unknown>) => {
          const report = item.stock_inventory_reports as Record<string, unknown>
          return managerIds.includes(report.manager_id as string)
        })

        const itemsMap: Record<string, { quantity: number; revenue: number }> = {}
        branchItems.forEach((item: Record<string, unknown>) => {
          const itemName = item.item_name as string
          if (!itemsMap[itemName]) itemsMap[itemName] = { quantity: 0, revenue: 0 }
          itemsMap[itemName].quantity += (item.quantity as number) || 0
          itemsMap[itemName].revenue += ((item.quantity as number) || 0) * ((item.unit_price as number) || 0)
        })

        const topItemEntry = Object.entries(itemsMap).sort(([, a], [, b]) => b.revenue - a.revenue)[0]
  ...
  const exportToExcel = () => {
    ...
    csv += `Branch,Total Revenue,Sales Revenue,Room Revenue,Laundry Revenue,Other Revenue,Avg Occupancy,Managers,Reports,Top Item\n`
    branches.forEach(branch => {
      ...
      csv += `₦${branch.otherRevenue.toLocaleString()},`
    ...
    <div className="p-8 max-w-7xl mx-auto">
          ...
          {([
            ...
            <button
              ...
              className={`px-4 py-3 rounded-lg font-medium transition-colors ${
                timeframe === option.value ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
              }`}
            >
              {option.label}
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div className="card">
          ...
          <div className="text-lg font-bold text-green-600">{branches[0]?.branchName || 'N/A'}</div>
          <div className="text-xs text-gray-500 mt-1">₦{(branches[0]?.totalRevenue || 0).toLocaleString('en-NG', { maximumFractionDigits: 0 })}</div>
        </div>
        <div className="card">
      ...
      <div className="space-y-6">
          ...
          <div key={index} className="card border-2 border-gray-200 hover:border-primary transition-colors">
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">{branch.managersCount} managers • {branch.reportsCount} reports</p>
                </div>
              </div>
              <Link
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-4 p-4 bg-gray-50 rounded-lg">
              <div>
                ...
                <div className="text-lg font-bold text-gray-900">₦{branch.totalRevenue.toLocaleString('en-NG', { maximumFractionDigits: 0 })}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500 mb-1">Sales (Food/Drinks)</div>
                <div className="text-lg font-bold text-orange-600">₦{branch.salesRevenue.toLocaleString('en-NG', { maximumFractionDigits: 0 })}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500 mb-1">Room Revenue</div>
                <div className="text-lg font-bold text-green-600">₦{branch.roomRevenue.toLocaleString('en-NG', { maximumFractionDigits: 0 })}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500 mb-1">Laundry</div>
                <div className="text-lg font-bold text-blue-600">₦{branch.laundryRevenue.toLocaleString('en-NG', { maximumFractionDigits: 0 })}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500 mb-1">Other Revenue</div>
                <div className="text-lg font-bold text-purple-600">₦{branch.otherRevenue.toLocaleString('en-NG', { maximumFractionDigits: 0 })}</div>
              </div>
            </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-purple-200">
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="text-purple-900 font-bold">₦{branch.topItem.revenue.toLocaleString('en-NG', { maximumFractionDigits: 0 })}</span>
                      </div>
                    </div>
                  </div>
                )}
        ...
        {branches.length === 0 && (
          <div className="card text-center py-12">
            ...
            <p className="text-gray-600">No approved reports found for the selected time period</p>
          </div>
        )}
      </div>
    </div>
