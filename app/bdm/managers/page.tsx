...
export default function ManagersPage() {
      ...
      if (managersData.length > 0) {
        ...
        for (const manager of managersData) {
          ...
          if (manager.role === 'manager') {
            // Managers now submit ONE unified stock_inventory_reports (includes stock + sales)
            // Plus expense reports if they still exist
            const [stockData, expenseData] = await Promise.all([
              supabase.from('stock_inventory_reports').select('status').eq('manager_id', manager.id),
            ...
            // Front office manager reports remain unchanged
  ...
  const getManagerRoleLabel = (role: string) =>
    role === 'front_office_manager' ? 'Front Office Manager' : 'Manager'

  if (loading) {
    return <div className="p-8"><div className="text-lg text-gray-600">Loading managers...</div></div>
  }
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <div className="text-xs text-gray-500 mt-1">
              {managers.filter(m => m.role === 'manager').length} Regular •{' '}
              {managers.filter(m => m.role === 'front_office_manager').length} Front Office
            </div>
          </div>
        </div>
      </div>
      ...
      {managers.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {managers.map((manager) => {
            const managerStats = stats.get(manager.id) || { total_reports: 0, pending: 0, approved: 0, rejected: 0 }
            return (
              <div key={manager.id} className="card hover:shadow-lg transition-shadow">
                <div className="flex items-start space-x-4 mb-4 pb-4 border-b border-gray-200">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-lg font-semibold text-gray-900 truncate">{manager.full_name}</h3>
                    ...
                    <div className="mt-1">
                      <span className={`text-xs px-2 py-1 rounded-full ${
                        manager.role === 'front_office_manager' ? 'bg-indigo-100 text-indigo-800' : 'bg-blue-100 text-blue-800'
                      }`}>
                ...
                {/* Report type labels per role */}
                <div className="mb-3 text-xs text-gray-500">
                  {manager.role === 'manager'
                    ? 'Submits: Stock & Sales (unified), Expenses'
                    : 'Submits: Occupancy, Guest Activity, Revenue, Complaints'}
                </div>
